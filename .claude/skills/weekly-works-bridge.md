...
description: churchTeam 백본에서 weekly-works 설교준비·주간콘텐츠 시스템을 호출하는 브릿지. 내부 변경 없이 경로 참조·호출 방법만 기술한다.
---

# Weekly-Works 브릿지

> ★절대 규칙: weekly-works 내부(sermon·research-bridge·team-leader) = 절대 수정 불가.
> 이 파일은 churchTeam → weekly-works 단방향 호출 인터페이스다.

## 경로

| 항목 | 절대 경로 |
|------|----------|
| weekly-works ROOT | `<workspace>/weekly-works/` |
...
## churchTeam → weekly-works 호출 시점

| churchTeam 팀 | 호출 조건 | 커맨드 |
|-----------------|----------|--------|
...
3. sermon-context.md 생성 완료 후 → churchTeam 월간기획·연간계획에 반영 가능
...
weekly-works 설교 완료
    ...
    → churchTeam 사역기획팀이 읽어 월간기획·소그룹·SNS 방향 정렬
```
