...
description: churchTeam 백본에서 church-admin 교회행정 시스템을 호출하는 브릿지. 내부 변경 없이 경로 참조·호출 방법만 기술한다.
---

# Church-Admin 브릿지

> ★절대 규칙: church-admin 내부 = 절대 수정 불가.
> 이 파일은 churchTeam → church-admin 단방향 호출 인터페이스다.

## 경로

| 항목 | 절대 경로 |
|------|----------|
| church-admin ROOT | `<workspace>/church-admin/` |
